    ...
    private void setUpDonutItems() {
        ...
        for (int i = 0; i < donutTypes.length; i++) {
            for (int j = 0; j < donutFlavors[i].length; j++) {
                donuts.add(
                        new Donut(donutTypes[i], donutFlavors[i][j], donutImages[i][j], Double.parseDouble(prices[i])));
            }
        }
    }
}
